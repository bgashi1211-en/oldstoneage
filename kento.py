...
# --- SIZE WINDOW: must land above 1.3GB and safely under 2GB ---
MIN_SIZE_BYTES = int(1.3 * 1024 ** 3)   # 1GB + 300MB floor
MAX_SIZE_BYTES = int(1.95 * 1024 ** 3)  # safe margin under GitHub's 2 GiB hard cap

AUDIO_BITRATE_K = 128
VIDEO_BITRATE_K = 4500  # 4.5 Mbps @ 1080p30

# Target comfortably inside the window so encoding variance never pushes
# the final file out of the accepted 1.3GB-1.95GB range
TARGET_SIZE_BYTES = random.randint(int(1.5 * 1024 ** 3), int(1.85 * 1024 ** 3))
...
def make_seamless_loop(src_path):
    ...
    if not src_path.exists():
        raise SystemExit(f"[FATAL] Source clip missing before loop step: {src_path}")

    clip_dur = probe_duration(src_path)
    if clip_dur <= 0:
        raise SystemExit(f"[FATAL] Clip duration invalid ({clip_dur}) for {src_path}")

    blend = min(LOOP_BLEND_SEC, clip_dur * 0.4)
    ...
    if proc.returncode != 0 or not out_path.exists() or out_path.stat().st_size == 0:
        print(proc.stdout)
        print(proc.stderr)
        raise SystemExit(f"[FATAL] Seamless loop generation failed (exit {proc.returncode}).")
...
if not clips_dir.exists():
    raise SystemExit(f"[FATAL] clips_dir does not exist after download: {clips_dir}")
if not audios_dir.exists():
    raise SystemExit(f"[FATAL] audios_dir does not exist after download: {audios_dir}")
...
=== RENDER JOB ===
  ...
  TARGET SIZE  : {TARGET_SIZE_BYTES / 1e9:.2f} GB   (window: 1.3GB - 1.95GB)
...
if final_size < MIN_SIZE_BYTES:
    raise SystemExit(f"[FATAL] Output only {final_size_gb:.3f} GB — below 1.3 GB minimum.")
